...
def get_financial_report(report_kind, ticker, market='us', read_data_dir=None, write_data_dir='data', variant='annual'):
    if pd.isnull(read_data_dir):
        logger.info(
            f"Reading {report_kind} dataset of market {market} from simfin api. Data will be written to '{write_data_dir}' directory")
        ...
        logger.info(f"Reading {report_kind} dataset of market {market} from '{read_data_dir}' directory")
        file_name = f"{market}-{report_kind}-{variant}.csv"
        report_file_path = os.path.join(read_data_dir, file_name)
